...
inline JoltPhysicsReactor::PoserMotionState::PoserMotionState(Poser& poser, const Model::RigidBody& body, const btTransform& body_transform)
  : poser_(poser),
    ...
    target_(JoltPhysicsReactor::GetPoserBoneImage(poser, body.GetAssociatedBoneIndex())),
...
inline void JoltPhysicsReactor::PoserMotionState::getWorldTransform(btTransform& transform) const {
    if(passive_) {
        Reset();
    }
...
inline void JoltPhysicsReactor::PoserMotionState::setWorldTransform(const btTransform& transform) {
    if(!passive_) {
        transform_ = transform;
    }
}

inline void JoltPhysicsReactor::PoserMotionState::Synchronize() {
    if(!(passive_||ghost_)) {
...
inline void JoltPhysicsReactor::PoserMotionState::Fix() {
    if(strict_) {
        ...
        if(target_.has_parent_) {
            parent_local_matrix = JoltPhysicsReactor::GetPoserBoneImage(poser_, target_.parent_).local_matrix_;
...
inline void JoltPhysicsReactor::PoserMotionState::Reset() const {
    transform_.setFromOpenGLMatrix(target_.skinning_matrix_.v);
    transform_ = transform_*body_transform_;
}


inline JoltPhysicsReactor::JoltPhysicsReactor() : configuration_(NULL), dispatcher_(NULL), broadphase_(NULL), solver_(NULL), world_(NULL), has_floor_(true) {
